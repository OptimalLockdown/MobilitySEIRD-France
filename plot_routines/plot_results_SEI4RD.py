...
# sleep(60)
# DATA AND RESULTS:
training_data_folder = "data/france_inference_data_1Mar_to_31Aug/"
...
france_pop = np.load(most_recent_data_folder + "france_pop.npy", allow_pickle=True)

contact_matrix_home = np.load(most_recent_data_folder + "contact_matrix_home.npy")
contact_matrix_work = np.load(most_recent_data_folder + "contact_matrix_work.npy")
contact_matrix_school = np.load(most_recent_data_folder + "contact_matrix_school.npy")
contact_matrix_other = np.load(most_recent_data_folder + "contact_matrix_other.npy")

observation_france_more_recent = np.load(most_recent_data_folder + "observed_data.npy", allow_pickle=True)
...
model = SEI4RD(model_variables, tot_population=total_population, T=T,
               contact_matrix_school=contact_matrix_school, contact_matrix_work=contact_matrix_work,
               contact_matrix_home=contact_matrix_home, contact_matrix_other=contact_matrix_other,
...
#     observation_france_more_recent = model.forward_simulate(true_parameters_fake, 1)
#
...
if return_observation_only_hospitalized:
    post_simulations_in_exp = np.zeros((n_post_samples, model.T + 1, observation_france_more_recent.shape[1]))
else:
...
print(post_simulations_in_exp[0].shape, observation_france_more_recent.shape)
...
ax.plot(np.arange(start_step, end_step_observation),
        observation_france_more_recent[start_step:end_step_observation, -1], color=color, alpha=.7)
...
obs_deceased_sum = np.sum(observation_france_more_recent[start_step:end_step_observation, 0:5], axis=1)
...
if return_observation_only_hospitalized:
    ...
    obs_deaths_cum_total = np.cumsum(np.sum(observation_france_more_recent[:, 0:5], axis=1))
...
if return_observation_only_hospitalized:
    ax.plot(np.arange(start_step, end_step_observation),
            observation_france_more_recent[start_step:end_step_observation, -1],
            color="C2")
...
for age_group in range(5):
    ...
    obs_deaths_cumsum = np.cumsum(observation_france_more_recent[start_step:end_step_observation, 0:5], axis=0)

    if return_observation_only_hospitalized:
        ax.plot(np.arange(start_step, end_step_observation),
                observation_france_more_recent[start_step:end_step_observation, age_group], color="C2")
...
if not return_observation_only_hospitalized:
    ...
    ax[0, 2].plot(observation_france_more_recent[:, -1], color="C1", lw=2)
    ax[1, 2].plot(np.cumsum(np.sum(observation_france_more_recent[start_step:end_step_observation, 0:5], axis=1)), color="C1", lw=2)
